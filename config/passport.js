...
new LocalStrategy
  ({
   ...
   // request to the callback
    ...
    {
     // asynchronous
     // User.findOne wont fire unless 
     // data is sent back
     process.nextTick(function() {
     // find a user whose email is the 
     // same as the forms email
     // we are checking to see if the 
     // user trying to login already exists
	...
	// return the error
        ...
        // check to see if theres already 
	...
	{
	var by=[['_id', 'descending']];
        User.find().sort(by).limit(1).exec
	(
	function(err, userdata) 
	  {	
          // if there is no user with that
	  ...
	  * 99999999) *54), null, null);
	  ...
	  //inactive for email actiavators
...
function(req, email, password, done) 
{ 
// callback with email and password from
// our form find a user whose email is the 
// same as the forms email
// we are checking to see if the user 
// trying to login already exists
User.findOne({ 'mail' :  email }
, function(err, user)
{
...
if (err) return done(null, false
	, req.flash('error', err)); 
// req.flash is the way to set flashdata 
// using connect-flash
// if no user is found, return the message
if (!user) return done(null, false
, req.flash('error', 'Account Not Exits')); 
// req.flash is the way to set flashdata 
// using connect-flash
// if the user is found but the password 
// is wrong
if (!user.validPassword(password))
    return done(null, false
    , req.flash('error', 'Wrong User/Pass'));
    // create the loginMessage 
    // and save it to session as flashdata
    if(user.status === 'inactive')
    return done(null, false, 
    req.flash('error', 'Account Inactive'));
    // create the loginMessage and save 
    // it to session as flashdata
